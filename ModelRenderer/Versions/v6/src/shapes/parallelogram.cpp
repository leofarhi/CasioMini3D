...
void DrawHorzParallelogramUV(VertexLocal &tl, VertexLocal &tr, VertexLocal &br, VertexLocal &bl)
{
    ...
    const fixed_t inv_height = fixed_t(1) / height;
    ...
    for (int y = startY; y <= endY; ++y) {
        const int x_st_int = (x_st);
        const int x_end_int = (x_end);
        ...
        if (length > 0) {
            const fixed_t inv_length = fixed_t(1) / length;
            const fixed_t du = (u_end - u_st* inv_length);
            const fixed_t dv = (v_end - v_st* inv_length);
...
void DrawHorzParallelogram(VertexLocal &tl, VertexLocal &tr, VertexLocal &br, VertexLocal &bl)
{
    ...
    const fixed_t dv = ((br.v - tl.v)* inv_height);
    fixed_t v = tl.v + dv * deltaStartY;

    for (int y = startY; y <= endY; ++y) {
        const int x_st_int = (int)(x_st);
        const int x_end_int = (int)(x_end);
        ...
        if (line_width > 0) {
            const fixed_t du = (br.u - tl.u* fixed_t(1) / line_width);
...
void DrawVertParallelogramUV(VertexLocal &tl, VertexLocal &tr, VertexLocal &br, VertexLocal &bl)
{
    ...
    const fixed_t inv_width = fixed_t(1) / width;
    const fixed_t du_top = ((tr.u - tl.u)* inv_width);
    const fixed_t dv_top = ((tr.v - tl.v)* inv_width);
    const fixed_t du_bottom = ((br.u - bl.u)* inv_width);
    const fixed_t dv_bottom = ((br.v - bl.v)* inv_width);

    const fixed_t dy_top = (fixed_t((tr.y - tl.y))* inv_width);
    const fixed_t dy_bottom = (fixed_t((br.y - bl.y))* inv_width);
    ...
    for (int x = startX; x <= endX; ++x) {
        const int y_top_int = (int)(y_top);
        const int y_bottom_int = (int)(y_bottom);
        ...
        if (length > 0) {
            const fixed_t inv_length = fixed_t(1) / length;
            const fixed_t du = ((u_bottom - u_top)* inv_length);
            const fixed_t dv = ((v_bottom - v_top)* inv_length);
...
void DrawVertParallelogram(VertexLocal &tl, VertexLocal &tr, VertexLocal &br, VertexLocal &bl)
{
    ...
    const fixed_t du = ((br.u - tl.u)* inv_width);
    ...
    for (int x = startX; x <= endX; ++x) {
        ...
        if (line_height > 0) {
            const fixed_t dv = ((br.v - tl.v)* (fixed_t(1) / line_height));
