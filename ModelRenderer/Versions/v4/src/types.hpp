...
Vector3<U> vec3_cast(const Vector3<T>& v);

// Implémentations
template<>
inline Vector3<fixed_t> vec3_cast<int, fixed_t>(const Vector3<int>& v) {
    return Vector3<fixed_t>{INT_TO_FIXED(v.x), INT_TO_FIXED(v.y), INT_TO_FIXED(v.z)};
}

template<>
inline Vector3<int> vec3_cast<fixed_t, int>(const Vector3<fixed_t>& v) {
    return Vector3<int>{TO_INT(v.x), TO_INT(v.y), TO_INT(v.z)};
}

template<>
inline Vector3<fixed_t> vec3_cast<float, fixed_t>(const Vector3<float>& v) {
    return Vector3<fixed_t>{FLOAT_TO_FIXED(v.x), FLOAT_TO_FIXED(v.y), FLOAT_TO_FIXED(v.z)};
}

template<>
inline Vector3<float> vec3_cast<fixed_t, float>(const Vector3<fixed_t>& v) {
    return Vector3<float>{TO_FLOAT(v.x), TO_FLOAT(v.y), TO_FLOAT(v.z)};
}

template<>
inline Vector3<int> vec3_cast<float, int>(const Vector3<float>& v) {
    return Vector3<int>{(int)v.x, (int)v.y, (int)v.z};
}

template<>
inline Vector3<float> vec3_cast<int, float>(const Vector3<int>& v) {
...
inline Vector3<fixed_t> cross(const Vector3<fixed_t>& a, const Vector3<fixed_t>& b) {
    return Vector3<fixed_t>{
