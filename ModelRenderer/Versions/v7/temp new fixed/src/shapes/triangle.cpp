...
inline Edge CalcEdge(const VertexLocal& a, const VertexLocal& b, int y)
{
    const int deltaY = b.y - a.y;
    if (deltaY == 0) return {a.x, y, a.u, a.v};

    fixed_t factor = fixed_t(y - a.y) / fixed_t(deltaY);
    int x = a.x + static_cast<int>(factor * (b.x - a.x));
    fixed_t u = a.u + (b.u - a.u) * factor;
    fixed_t v = a.v + (b.v - a.v) * factor;
