...
Mesh* LoadCube()
{
    ...
    mesh->vertices[0].position = Vector3<float>{-1, -1, -1};
    mesh->vertices[1].position = Vector3<float>{1, -1, -1};
    mesh->vertices[2].position = Vector3<float>{1, 1, -1};
    mesh->vertices[3].position = Vector3<float>{-1, 1, -1};
    mesh->vertices[4].position = Vector3<float>{-1, -1, 1};
    mesh->vertices[5].position = Vector3<float>{1, -1, 1};
    mesh->vertices[6].position = Vector3<float>{1, 1, 1};
    mesh->vertices[7].position = Vector3<float>{-1, 1, 1};
    mesh->uvs[0] = Vector2<fixed_t>{0, 0};
    mesh->uvs[1] = Vector2<fixed_t>{256, 0};
    mesh->uvs[2] = Vector2<fixed_t>{256, 256};
    mesh->uvs[3] = Vector2<fixed_t>{0, 256};
    mesh->faces[0] = Face(4, (unsigned int[]){0, 1, 2, 3}, (unsigned int[]){0, 1, 2, 3}, NULL);
    mesh->faces[1] = Face(4, (unsigned int[]){1, 5, 6, 2}, (unsigned int[]){0, 1, 2, 3}, NULL);
    mesh->faces[2] = Face(4, (unsigned int[]){5, 4, 7, 6}, (unsigned int[]){0, 1, 2, 3}, NULL);
    mesh->faces[3] = Face(4, (unsigned int[]){4, 0, 3, 7}, (unsigned int[]){0, 1, 2, 3}, NULL);
    mesh->faces[4] = Face(4, (unsigned int[]){3, 2, 6, 7}, (unsigned int[]){0, 1, 2, 3}, NULL);
    mesh->faces[5] = Face(4, (unsigned int[]){4, 5, 1, 0}, (unsigned int[]){0, 1, 2, 3}, NULL);
    return mesh;
}
...
int main(){
    while (!keydown(KEY_MENU))
    {
        dclear(C_BLACK);
        clearevents();
        VertexLocal tl = {fixed_t(10), fixed_t(10), fixed_t(0), fixed_t(0)};
        VertexLocal tr = {fixed_t(200), fixed_t(10), fixed_t(255), fixed_t(0)};
        VertexLocal br = {fixed_t(200), fixed_t(200), fixed_t(255), fixed_t(255)};
        VertexLocal bl = {fixed_t(10), fixed_t(200), fixed_t(0), fixed_t(255)};
...
int main2(){
    ...
    while (!keydown(KEY_MENU))
    {
        ...
        Vector3<fixed_t> offset = {fixed_t(0), fixed_t(0), fixed_t(0)};
