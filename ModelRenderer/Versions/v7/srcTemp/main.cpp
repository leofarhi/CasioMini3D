...
Mesh* LoadModel(char* path)
{
    ...
    for (unsigned int i = 0; i < vertexCount; i++)
    {
        ...
        Vector3<float> position = {x*80, y*80, z*80};
        mesh->vertices[i].position = vec3_cast<float, fixed_t>(position);
...
{
    Vector3<float> vf = vec3_cast<fixed_t, float>(v);
...
{
    /*const fixed_t moveSpeed = FLOAT_TO_FIXED(25.0f);
    const fixed_t rotSpeed = FLOAT_TO_FIXED(0.2f);*/
    //adaptation des vitesses en fonction de deltaTime
    float dt = (float)deltaTime / CLOCKS_PER_SEC;
    const fixed_t moveSpeed = FLOAT_TO_FIXED(160.0f * dt);
    const fixed_t rotSpeed = FLOAT_TO_FIXED(0.8f * dt);
    ...
    if (keydown(KEY_7))
        camera->position += fmul(forward, moveSpeed);
    if (keydown(KEY_1))
        camera->position -= fmul(forward, moveSpeed);
    if (keydown(KEY_6))
        camera->position += fmul(right, moveSpeed);
    if (keydown(KEY_4))
        camera->position -= fmul(right, moveSpeed);
    if (keydown(KEY_8))
        camera->position += fmul(up, moveSpeed);
    if (keydown(KEY_2))
        camera->position -= fmul(up, moveSpeed);
...
int main(){
    ...
    camera->position = vec3_cast<int, fixed_t>(Vector3<int>{0, 235, -475});
