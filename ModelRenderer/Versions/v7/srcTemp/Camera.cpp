...
Camera::Camera()
{
    if (Camera::current == nullptr)
        Camera::current = this;
    this->fov = INT_TO_FIXED(SCREEN_WIDTH);
...
bool IsBackface(Vertex* vertices, Face* face)
{
    Vector3<fixed_t> v1 = vertices[face->vertices[1]].position - vertices[face->vertices[0]].position;
    Vector3<fixed_t> v2 = vertices[face->vertices[2]].position - vertices[face->vertices[0]].position;
    Vector3<fixed_t> normal = v1.cross(v2);
    Vector3<fixed_t> view = vertices[face->vertices[0]].position - Camera::current->position;
    return normal.dot(view) > 0;
}

bool IsVisible(RenderFace face)
{
    //Check Depth
    if (face.depth <= 0)
        return false;
    //Check if point is outside the screen
    bool inside = false;
    for (int i = 0; i < face.face->length; i++)
    {
        if (face.mesh->vertices[face.face->vertices[i]].projected.x >= 0 &&
            face.mesh->vertices[face.face->vertices[i]].projected.x < INT_TO_FIXED(SCREEN_WIDTH) &&
            face.mesh->vertices[face.face->vertices[i]].projected.y >= 0 &&
            face.mesh->vertices[face.face->vertices[i]].projected.y < INT_TO_FIXED(SCREEN_HEIGHT))
        {
            inside = true;
            break;
        }
    ...
    if (!inside)
        return false;
    //Check if face is backface
    return true;//!IsBackface(face.mesh->vertices, face.face);
}
...
{
    VertexLocal verts[4];
    const int texW_minus1 = 256-1;
    const int texH_minus1 = 256-1;
