...
static inline fixed_t sqrt_fixed(fixed_t x) {
    // Convertir en float, sqrt, retransformer : rapide et assez précis
    return FLOAT_TO_FIXED(sqrtf(TO_FLOAT(x)));
}

static inline fixed_t finv(fixed_t x) {
    return fdiv(FIXED_ONE, x);
}
