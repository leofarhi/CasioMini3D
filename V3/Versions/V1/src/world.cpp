...
/*void DrawColoredQuad(RenderQuad quad, int color)
{
    ...
    struct VertexLocal { int x, y; fixed_t u, v; } verts[4];
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        verts[i].u = FLOAT_TO_FIXED(uv[i][0] * (texW - 1));
        verts[i].v = FLOAT_TO_FIXED(uv[i][1] * (texH - 1));
    }

    auto DrawTriangle = [&](VertexLocal v0, VertexLocal v1, VertexLocal v2) {
        ...
        for (int y = startY; y <= endY; ++y) {
            ...
            int Ax = v0.x + TO_INT((v2.x - v0.x) * alpha);
            int Bx = second_half ? v1.x + TO_INT((v2.x - v1.x) * beta) : v0.x + TO_INT((v1.x - v0.x) * beta);

            fixed_t Au = v0.u + ((v2.u - v0.u) * alpha >> PRECISION);
            fixed_t Av = v0.v + ((v2.v - v0.v) * alpha >> PRECISION);
            fixed_t Bu = second_half ? v1.u + ((v2.u - v1.u) * beta >> PRECISION) : v0.u + ((v1.u - v0.u) * beta >> PRECISION);
            fixed_t Bv = second_half ? v1.v + ((v2.v - v1.v) * beta >> PRECISION) : v0.v + ((v1.v - v0.v) * beta >> PRECISION);
            ...
            int lastU = -1, lastV = -1, pixelColor = color;
            ...
            for (int x = startX; x <= endX; ++x) {
                ...
                if (currU != lastU || currV != lastV) {
                    pixelColor = get_uv_map(currU, currV);
                    lastU = currU;
                    lastV = currV;
                }
...
inline std::tuple<int, fixed_t, fixed_t> CalcEdge(VertexLocal a, VertexLocal b, int y)
{
    int deltaY = b.y - a.y;
    int factor = deltaY ? ((y - a.y) << PRECISION) / deltaY : 0;
    int x = a.x + TO_INT((b.x - a.x) * factor);
    fixed_t u = a.u + ((b.u - a.u) * factor >> PRECISION);
    fixed_t v = a.v + ((b.v - a.v) * factor >> PRECISION);
    return {x, u, v};
};
...
void DrawTriangle(VertexLocal v0, VertexLocal v1, VertexLocal v2) {
    ...
    for (int y = startY; y <= endY; ++y) {
        ...
        auto [Ax, Au, Av] = CalcEdge(v0, v2, y);
        auto [Bx, Bu, Bv] = second_half ? CalcEdge(v1, v2, y) : CalcEdge(v0, v1, y);
        ProcessScanline(y, Ax, Bx, Au, Av, Bu, Bv);
    }
};
