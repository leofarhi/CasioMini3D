...
void DrawColoredQuad(RenderQuad quad, int color) {
    ...
    else
    {
        ...
        /*for (int y = y_limits.x; y < y_limits.y; y ++)
        {
            intmax_t left_x = INT_MAX;
            intmax_t right_x = INT_MIN;
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                const fixed_t fy = INT_TO_FIXED(y);

                if ((fy >= p1.y && fy <= p2.y) || (fy >= p2.y && fy <= p1.y))
                {
                    int x = TO_INT(intersect_y(p1, p2, fy));
        ...
        /*for (int y = y_limits.x; y < y_limits.y; y++)
        {
            fixed_t x_left = intersect_y(top_left,bottom_left , INT_TO_FIXED(y));
            fixed_t x_right = intersect_y(top_right, bottom_right, INT_TO_FIXED(y));
            if (x_right != x_left)
            {
                fixed_t du = fdiv(uvmap[1].x - uvmap[0].x, x_right - x_left);
                fixed_t u = uvmap[0].x + fmul(du, (x_left - top_left.x));
                fixed_t dv = fdiv(uvmap[1].y - uvmap[0].y, y_limits.y - y_limits.x);
                fixed_t v = uvmap[0].y + fmul(dv, (y - y_limits.x));
                for (int x = TO_INT(x_left); x < TO_INT(x_right); x++)
                {
                    int u_int = TO_INT(u)%40;
                    int v_int = TO_INT(v)%40;
                    int color = get_uv_map(u_int, v_int);
        ...
        for (int y = y_limits.x; y < y_limits.y; y ++)
        {
            fixed_t x_left = INT_MAX;
            fixed_t x_right = INT_MIN;
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if ((fy >= p1.y && fy <= p2.y) || (fy >= p2.y && fy <= p1.y))
                {
                    fixed_t x = intersect_y(p1, p2, fy);
